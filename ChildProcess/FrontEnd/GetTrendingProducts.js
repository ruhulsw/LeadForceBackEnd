...
async function handleTrendingProductsRequest(req) {
  const url = req.url;
  ...
  if (cachedData) {
    console.log("==>> Cache Trending-Products");
    ...
    const products = await Product.find({ uid: getDom.uid, Trending: "1" })
      .limit(10)
      .sort({ updateDate: -1 });
...
process.on("message", async (req) => {
  try {
    const result = await handleTrendingProductsRequest(req);
    process.send(result);
  } catch (error) {
